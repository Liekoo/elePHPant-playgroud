...
<div class="card">
  ...
  <form method="POST">
    ...
    <div class="form-grid">
      <div class="form-group"><label>Product Name</label><input type="text" name="Product_Name" required value="<?= htmlspecialchars($editRow['Product_Name'] ?? '') ?>"></div>
      <div class="form-group"><label>Price (₱)</label><input type="number" name="Product_Price" step="0.01" min="0" required value="<?= $editRow['Product_Price'] ?? '' ?>"></div>
      <div class="form-group"><label>Stock</label><input type="number" name="Product_Quantity_Stock" min="0" required value="<?= $editRow['Product_Quantity_Stock'] ?? 0 ?>"></div>
      ...
      <div class="form-group full"><label>Description</label><textarea name="Product_Description"><?= htmlspecialchars($editRow['Product_Description'] ?? '') ?></textarea></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><a href="?edit=<?= $row['Product_ID'] ?>" class="btn btn-ghost btn-sm">Edit</a></td>
        </tr>
        <?php endwhile; ?>
      </tbody>
    </table>
