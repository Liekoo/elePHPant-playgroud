...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['Order_ID']) && $_POST['Order_ID'] !== '') {
        ...
        $conn->query("
            INSERT INTO orders (Product_ID, Size_ID, Customer_Type_ID, Payment_Type_ID, Order_Quantity, Product_Price, Order_Status)
            VALUES ($product_id, $size_id, $customer_type_id, $payment_type_id, $order_quantity, $product_price, '$order_status')
        ");
...
$orders = $conn->query("
    SELECT o.*, p.Product_Name, s.Size_Label, s.Size_Name,
...
<?php if (isset($_GET['success'])): ?>
  <div class="alert alert-success">✓ Order <?= htmlspecialchars($_GET['success']) ?> successfully.</div>
<?php endif; ?>
...
<div class="card">
  ...
  <form method="POST">
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        <label>Size</label>
        <select name="Size_ID" id="sizeSelect" onchange="updatePrice(document.getElementById('productSelect'))">
          <option value="">— No size —</option>
          <?php while ($sz = $sizes->fetch_assoc()): ?>
            <option value="<?= $sz['Size_ID'] ?>" data-addon="<?= $sz['Size_Price'] ?>"
                    <?= ($editRow && $editRow['Size_ID']==$sz['Size_ID']) ? 'selected' : '' ?>>
      ...
      <div class="form-group">
        ...
        <input type="number" name="Order_Quantity" min="1" required
               value="<?= $editRow['Order_Quantity'] ?? 1 ?>" oninput="updateTotal()">
      </div>
    ...
    <div class="form-actions">
      <button type="submit" class="btn btn-primary"><?= $editRow ? '✓ Update Order' : '+ Add Order' ?></button>
      <?php if ($editRow): ?><a href="orders.php" class="btn btn-ghost">Cancel</a><?php endif; ?>
    </div>
  </form>
</div>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            <a href="?delete=<?= $row['Order_ID'] ?>" class="btn btn-danger btn-sm"
...
<script>
let currentPrice = <?= $editRow ? floatval($editRow['Product_Price']) : 0 ?>;
...
function updatePrice(sel) {
  ...
  const addon = parseFloat(sizeOpt?.dataset?.addon) || 0;
  currentPrice = base + addon;
